...
public class EyeDraw {
     ...
     */
    public static void drawContact(Canvas canvas, Bitmap contactBitmap,
                                   Path eyePath, Point centerPoint,
                                   int eyeRadius, int alpha) {
        Path contactPath = new Path();
        contactPath.addCircle(centerPoint.x,centerPoint.y,
                eyeRadius, Path.Direction.CCW);
        ...
        canvas.drawBitmap(contactBitmap,new Rect(0,30,contactBitmap.getWidth(),
                contactBitmap.getHeight() - 60),bounds,paint);
    }

    /**
     * eyelash
     */
    public static void drawLash(Context context, Canvas canvas,
                                EyeAngleAndScaleCalc.Bean bean, List<Point> pointList,
                                int alpha, boolean needMirror) {
        ...
        Bitmap scaledBitmapTop = Bitmap.createScaledBitmap(resTopBitmap,
                (int) (resTopBitmap.getWidth() * eyeAngleAndScaleCalc.topScaleX + 0.5),
        ...
        if (!TextUtils.isEmpty(bean.resBottom)) {
            ...
            scaledBitmapBottom = Bitmap.createScaledBitmap(resBottomBitmap,
                    (int) (resBottomBitmap.getWidth() * eyeAngleAndScaleCalc.bottomScaleX + 0.5),
        ...
        if (needMirror) {
            ...
            scaledBitmapTop = Bitmap.createBitmap(scaledBitmapTop, 0, 0,
                    scaledBitmapTop.getWidth(),
                    scaledBitmapTop.getHeight(), matrix, true);
            if (resBottomBitmap != null) {
                scaledBitmapBottom = Bitmap.createBitmap(scaledBitmapBottom, 0, 0,
                        scaledBitmapBottom.getWidth(), scaledBitmapBottom.getHeight(),
                        matrix, true);
            }
        }

        canvas.save();
        //canvas.rotate(eyeAngleAndScaleCalc.getTopEyeAngle(),
        // eyeAngleAndScaleCalc.topP1.x, eyeAngleAndScaleCalc.topP1.y);
        ...
        if (scaledBitmapBottom != null) {
            canvas.save();
            canvas.rotate(eyeAngleAndScaleCalc.getBottomEyeAngle(),
                    eyeAngleAndScaleCalc.bottomP1.x, eyeAngleAndScaleCalc.bottomP1.y);
     ...
     *
     */
    public static void drawShadow(Context context, Canvas canvas, EyeAngleAndScaleCalc.Bean bean,
                                  Path eyePath, List<Point> pointList, int alpha) {
        Paint paint = new Paint();
        paint.setAlpha(alpha);
        EyeAngleAndScaleCalc eyeAngleAndScaleCalc = new EyeAngleAndScaleCalc(pointList,bean);
        ...
        Matrix matrix = new Matrix();
        matrix.postScale(-1, 1);   //镜像水平翻转
        scaledBitmapLower = Bitmap.createBitmap(scaledBitmapLower, 0, 0, scaledBitmapLower.getWidth(),
                scaledBitmapLower.getHeight(), matrix, true);


        canvas.rotate(eyeAngleAndScaleCalc.getTopEyeAngle(), point.x, point.y);
        canvas.drawBitmap(scaledBitmapLower, point.x - (bitmapShadow.getWidth() - bean.topP3.x) * xScale,
                point.y - bean.topP3.y * yScale, paint);
        canvas.restore();
